...
flowchart LR
    ...
    A2["Fake Store REST API"] --> B2["ingestion/load_fake_store_api.py"]
...
non-technical stakeholders. I also added a small separate REST-ingestion
script against a public API as a second 'live ingestion' proof point."
...
| "Why didn't you fold the Fake Store API data into the star schema?" | It's a deliberately separate, small proof point for live REST ingestion — mixing unrelated product data into the core fact tables would contaminate the metrics with data that doesn't belong to the same business. |
